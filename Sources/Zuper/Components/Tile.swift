...
public enum TileDisclosure {
    ...
    case buttonLink(_ label: String, style: ButtonLink.ColorStyle = .primary)
}
...
public extension Tile {
    ...
    /// Creates Zuper Tile component with custom content.
    ///
    /// - Parameters:
...
struct TilePreviews: PreviewProvider {
        ...
        VStack(spacing: .medium) {
            StateWrapper(initialState: CGFloat(0)) { state in
                ContentHeightReader(height: state) {
                    Tile("Height \(state.wrappedValue)", description: description, icon: gridIcon)
                }
            }
            StateWrapper(initialState: CGFloat(0)) { state in
                ContentHeightReader(height: state) {
                    Tile("Height \(state.wrappedValue)", icon: gridIcon)
                }
            }
            StateWrapper(initialState: CGFloat(0)) { state in
                ContentHeightReader(height: state) {
                    Tile(description: "Height \(state.wrappedValue)", icon: gridIcon)
                }
            }
            StateWrapper(initialState: CGFloat(0)) { state in
                ContentHeightReader(height: state) {
    ...
    static var standalone: some View {
        Tile(title, description: description, icon: gridIcon)
    ...
    @ViewBuilder static var storybookMix: some View {
        VStack(spacing: .large) {
            Tile("Title with very very very very very long multiline text", description: descriptionMultiline, icon: .alert) {
                contentPlaceholder
            }
            Tile(title, description: description, icon: gridIcon, status: .info)
            ...
            Tile(title, description: description, icon: .alertCircle, disclosure: .icon(gridIcon))

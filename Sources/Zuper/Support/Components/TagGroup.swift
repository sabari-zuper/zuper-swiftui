...
/// Collection of Zuper tags with binding through `TagModel` protocol.
///
...
public struct TagGroup<TM: TagModel>: View {
    ...
    /// Creates a collection of related Zuper Tag components.
    public init(
        label: String = "",
        tags: Binding<[TM]>,
                ...
                StateWrapper(
                    ...
                    VStack(alignment: .leading) {
                        ...
                        ForEach(tags.wrappedValue) { tag in
                            HStack {
                                Icon(content: tag.isSelected ? .checkCircle : .circleEmpty, size: .small)
                                Icon(content: tag.isRemoved ? .remove : .circleEmpty, size: .small)
